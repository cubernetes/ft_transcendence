...
export default class UserController {
    ...
    async register(request: FastifyRequest<{ Body: UserInsertBody }>, reply: FastifyReply) {
        try {
            const { username, password, displayName } = request.body;

            const user = await this.authService.register(username, password, displayName);
            return reply.code(201).send({ user, message: `User registered successfully` });
        } catch (error) {
    ...
    async login(request: FastifyRequest<{ Body: UserLoginBody }>, reply: FastifyReply) {
        try {
